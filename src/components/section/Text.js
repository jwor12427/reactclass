...
const textInfo = [
	{
		infoTitle: "금강앵무",
		infoDesc:
			"금강앵무는 앵무새는 대부분의 매체에서 앵무새 하면 나오는 앵무새입니다. 깃털색이 화려하며, 크기가 대형종이 속하는 앵무새 입니다.",
		infoName: "t1",
		infoLink: "/",
	},
	{
		infoTitle: "회색앵무",
		infoDesc:
			"회색앵무는 앵무새중 가장 지능이 높다고 알려진 새입니다. 세계에서 가장 유명한 회색앵무는 이렌느 페퍼버그 박사의 알렉스입니다.",
		infoName: "t2",
		infoLink: "/",
	},
	{
	...
	},
	{
		infoTitle: "코뉴어",
		infoDesc:
			"가장 대중적인 앵무새이며, 애교가 많고 사람을 잘 따르는 앵무새 중 손꼽힙니다. 주로 좁은 곳이나 틈새를 파고 드는 습성이 있습니다.",
		infoName: "t4",
		infoLink: "/",
	},
	{
		infoTitle: "장미앵무",
		infoDesc:
			"한국에서 많이 키우는 종류 중 하나이며, 화려한 색채 때문에 애완용으로 인기가 많습니다. 하지만 겁이 많고 예민한 성격에 자주 데리고 놀면 스트레스를 받습니다.",
		infoName: "t5",
		infoLink: "/",
	},
	{
		infoTitle: "모란앵무",
		infoDesc:
			"모란앵무는 성격이 매우 활달하고 호기심이 많아 부리를 써서 탐색하는 것을 좋아합니다. 주인과 유대감이 매우 깊을 때는 주인의 옆에 꼭 붙어 있는 것이 특징입니다.",
		infoName: "t6",
		infoLink: "/",
	},
];

const TextMain = ({ infoTitle, infoDesc, infoName, infoLink }) => {
	return (
		<div className={`text ${infoName}`}>
			<h3 className="text__title">{infoTitle}</h3>
			<p className="text__desc">{infoDesc}</p>
			<a
				className="text_btn"
				href={infoLink}
			>
...
function Text({ attr }) {
	return (
		<section
			id="textType"
			className={`text__wrap ${attr[0]} ${attr[1]}`}
		>
